...
async function createUserProfile(userName, userAttributes) {
    try {
        ...
        if (process.env.USER_PROFILES_TABLE) {
            const dynamodb = new AWS.DynamoDB.DocumentClient();
            await dynamodb.put({
                TableName: process.env.USER_PROFILES_TABLE,
                Item: profile
            }).promise();
            console.log('User profile stored in DynamoDB');
        }
...
async function sendWelcomeNotification(email, role) {
    try {
        ...
        if (process.env.WELCOME_NOTIFICATIONS_TOPIC) {
            const sns = new AWS.SNS();
            await sns.publish({
                TopicArn: process.env.WELCOME_NOTIFICATIONS_TOPIC,
                ...
                Subject: 'Welcome to AWS Education Platform'
            }).promise();
            console.log('Welcome notification sent via SNS');
        }
